var user = {
    ...
    requestUrl_addUser : constant.ssoInterfaceReg(),
    /**
    ...
    textarealength : function(obj, length){
    	    var desc = obj.value;
    	    obj.value = user.substr(obj.value,length);
    },
    ...
    addUserSubmit : function(){
    	ajax.getJsonp(user.requestUrl_addUser+"?userName="+$("#username").val()+
    			"&sex="+$('input:radio:checked').val()+"&mobile="+$("#mobile").val()+
    			"&email="+$("#email").val()+"&remark="+$("#textareaId").val()+
    			"&userId="+0,"user.addUserCallBack") ;
